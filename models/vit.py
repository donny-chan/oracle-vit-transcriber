...
class Attention(nn.Module):
    ...
    def forward(self, x, register_hook=False):
        ...
        # (3, B, num_heads, N, head_dim) = (3, 4, 8, 65, 32)
...
class Block(nn.Module):
    ...
    def forward(self, x, register_hook=False):
        h = self.norm1(x)
        h = self.attn(h, register_hook=register_hook)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            representation_size (Optional[int]):
                enable and set representation layer (pre-logits) to this value
                if set
    ...
    def forward(self, x, register_blk=-1):
        """
        x: (B, L, channel, height, width)

        Return Tuple[
            embedding: (B, L, P, embed_dim)
            all_layer_inputs: List[
                (B, L, P, embed_dim)
            ]
        ]
        """
        batch_size = x.shape[0]
        x = self.patch_embed(x)  # (B, L, # patch, # )
        ...
        for i, blk in enumerate(self.blocks):
            all_layer_inputs.append(x)
            x = blk(x, register_blk == i)
        x = self.norm(x)

        return x, all_layer_inputs
