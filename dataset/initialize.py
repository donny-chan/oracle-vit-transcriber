...
from dataset.complete_dataset import (
    OracleCompleteDataset,
    OracleCompleteSingleDataset,
)
...
from dataset.transcription_dataset import TranscriptionDataset


def create_dataset(dataset, mode, config, tokenizer=None):
    if dataset == "pretrain":
        dataset = OracleCompleteDataset(
            config, mode, tokenizer, add_mask=False
        )
        return dataset
        ...
        dataset = OracleCompleteSingleDataset(
            config, mode, tokenizer, add_mask=True
        )
        return dataset
        ...
        return dataset
    elif dataset == "transcribe":
        return TranscriptionDataset(config, mode)
    else:
...
def pretrain_collate_fn(batch, tokenizer):
    ...
    for img, input_ids in batch:
        ...
        input_ids = torch.cat(
            (input_ids, torch.LongTensor([tokenizer.pad_token_id] * pad_len)),
            dim=0,
        )
    ...
    return (
        ...
        torch.cat(sentences, dim=0),
        torch.FloatTensor(attn_masks),
    )
...
def create_position_ids_from_input_ids(
    ...
    # The series of casts and type-conversions here are carefully balanced to
    # both work with ONNX export and XLA.
...
def mlm_single_collate_fn(batch, tokenizer, modality, img_pad_color=1.0):
    ...
    [CLS], text tokens * n1, [SEP], [PAD] * n2, image embeds * n1,
    [IMG_PAD] * n2

    return: image_matrix with padding, input_ids with cls+seq+padding,
    attn_mask (both), labels (both)
    """
    (
        images,
        sentences,
    ...
    for (
        ...
        img = torch.cat(
            (img, torch.full((n2, img.shape[1]), img_pad_color)), dim=0
        )
        images.append(img.unsqueeze(0))
        # [batch_size, cls + n1 + sep + n2]
        input_ids = torch.cat(
            (input_ids, torch.LongTensor([tokenizer.pad_token_id] * n2)), dim=0
        )
        sentences.append(input_ids.unsqueeze(0))
        if modality == "cross":
            # [batch_size, cls + n1 + sep + n2 + n1 + n2]
            label = torch.cat(
                (label, torch.LongTensor([-100] * (max_len + n2))), dim=0
            )
            plain_label = torch.cat(
                (plain_label, torch.LongTensor([-100] * (max_len + n2))), dim=0
            )
            attn_mask = [1] * (n1 + 2) + [0] * n2 + [1] * n1 + [0] * n2
        elif modality == "text":
            label = torch.cat((label, torch.LongTensor([-100] * n2)), dim=0)
            plain_label = torch.cat(
                (plain_label, torch.LongTensor([-100] * n2)), dim=0
            )
            attn_mask = [1] * (n1 + 2) + [0] * n2
        else:
            label = torch.cat(
                (label[1:-1], torch.LongTensor([-100] * n2)), dim=0
            )
            plain_label = torch.cat(
                (plain_label[1:-1], torch.LongTensor([-100] * n2)), dim=0
            )
            attn_mask = [1] * n1 + [0] * n2
        labels.append(label.unsqueeze(0))
    ...
    temp_pos = create_position_ids_from_input_ids(
    ...
    for bid, (n1, n2) in enumerate(lengths):
        ...
        if modality == "cross":
            # cls + n1 + sep + n2 + n1 + n2
            pos_ids.append(
                cls_pos + word_pos + pad_pos + word_pos[:-1] + pad_pos
            )
    ...
    return (
        torch.cat(images, dim=0),
        torch.cat(mask_ori_images, dim=0),
        input_ids,
        torch.FloatTensor(attn_masks),
        torch.cat(labels, dim=0),
        torch.cat(plain_labels, dim=0),
        torch.LongTensor(pos_ids),
        torch.LongTensor(type_ids),
        lengths,
        book_orders,
        torch.LongTensor(mask_ids),
        torch.LongTensor(mask_img_ids),
        mask_chs,
    )
...
def image_reconstruct_collate_fn(batch, img_pad_color=1.0):
    ...
    for image, label, text in batch:
        ...
        image = torch.cat(
            (image, torch.full((pad_len, image.shape[1]), img_pad_color)),
            dim=0,
        )
        images.append(image.unsqueeze(0))
        label = torch.cat(
            (label, torch.full((pad_len, label.shape[1]), img_pad_color)),
            dim=0,
        )
        labels.append(label.unsqueeze(0))
...
def sharpen_unet_collate_fn(batch, config, mode):
    ...
    for noise_path, label_path in batch:
        ...
        if mode != "test":
            ...
            assert noise_img.shape == label_img.shape == pad_shape, (
                str(noise_img.shape) + " " + str(label_img.shape)
            )
        else:
    ...
    return {
        ...
        "labels": torch.FloatTensor(label_batch)
        if mode != "test"
        else label_batch,
    }
